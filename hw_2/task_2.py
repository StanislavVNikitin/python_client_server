...
def write_order_to_json(item,quantity, price, buyer, date):
    with open('orders.json', encoding='utf-8') as f_n:
        DATA_JSON = json.load(f_n)

    my_dict = {'item':item,'quantity':quantity, 'price':price, 'buyer':buyer,'date':date }
    DATA_JSON['orders'].append(my_dict)
    with open('orders.json', 'w', encoding='utf-8') as f_n:
        json.dump(DATA_JSON, f_n, indent=4)



write_order_to_json('New Item',7,80,'Ivan Ivanovich','10-10-2022')
write_order_to_json('New Item Next',1,33,'Petr Petrovich','11-10-2022')